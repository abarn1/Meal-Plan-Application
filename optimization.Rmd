...
```{r}
nutrition <- read_csv("D:/OneDrive/Documents/DA401/data/BFPD_csv_07132018/nutrient.csv")
food <- read_csv("D:/OneDrive/Documents/DA401/data/BFPD_csv_07132018/Products.csv")
...
for (i in lpindex){
  ...
  lpndb <- c(lpndb, ndb_no[i,])
}
lpfood <- list()
lpnut <- matrix()

for (i in lpndb){
  lpfood <- c(lpfood, match(i,food$NDB_Number))
}
lpfoodname <- list()
count = 0
for (i in lpfood){
  count = count + 1
  lpfoodname <- c(lpfoodname, food$long_name[i], lpamount[count])
}
lpend = Sys.time()
...
gurndb <- list()
for (i in gurindex){
  gurndb <- c(gurndb, ndb_no[i,])
}
gurfood <- list()
for (i in gurndb){
  gurfood <- c(gurfood, match(i,food$NDB_Number))
}
gurfoodname <- list()
count = 0
for (i in gurfood){
  count = count + 1
  gurfoodname <- c(gurfoodname, food$long_name[i], guramount[count])
}
gurend = Sys.time()
